...
suppressMessages(
  bind_rows(
  read_excel(paste0("data/final/final_final/final_final_final/", "MP Nesting Summary 2020_21 to 2006_07 fix up post GM meeting 15_2_24.xlsx"), 
...
nest_data_MP <-
  bind_rows(
    lucinda_nest_import(year_1 = "2020", year_2 = "2021", 
                        file_name = "MP Nesting Summary 2020_21 to 2006_07 fix up post GM meeting 15_2_24.xlsx", site = "MP",
...
suppressMessages(bind_rows(
  read_excel(paste0("data/final/final_final/final_final_final/", "FP Nesting summary 2020_21 to 2009_10 FINAL- checking names consistent for threat data.xlsx"), 
...
```{r}
suppressMessages(bind_rows(
  read_excel(paste0("data/final/final_final/final_final_final/", "Bellarine_Surf Coast Nesting with site names matching with threat data doc.xlsx"), 
...
determine the 99% quantile limit for each threat (i.e., to remove outlier data) - shown as the red verticle line here
```{r}
...
how do predatory bird counts vary over the week?
```{r}
mod_pred_birds <- 
...
Inspect the weekly varition in human prints
```{r}
...
Inspect the weekly varition in dog prints
```{r}
...
Inspect the weekly varition in vehicle prints
```{r}
...
Inspect the weekly varition in fox prints
```{r}
...
threat_data_99_ql_ <- 
  nest_data_with_threat_data_7d %>% 
  summarise_at(c("hum_a", "veh_a", "dog_a", "don_a", "dof_a", 
                 "hum_m", "veh_m", "dog_m", "don_m", "dof_m",
...
nest_survival_FP <- function()
{
  ...
  # average predatory birds detected and management levels
  S.pbd_a_level <-
  ...
  # max predatory birds detected and management levels
  S.pbd_m_level <-
...
saveRDS(nest_survival_FP, file = "output/nest_survival_FP.rds")
```
...
saveRDS(nest_survival_MP, file = "output/nest_survival_MP.rds")
```
...
saveRDS(nest_survival_BSC, file = "output/nest_survival_BSC.rds")
```

# Results
Only the top 10 models are shown here in each table for simplicity
```{r}
...
nest_survival_reals_FP_levels <- 
  nest_survival_run_FP[[36]]$results$real
Groups <- data.frame(
...
nest_survival_reals_MP_levels <- 
  nest_survival_run_MP[[35]]$results$real
Groups <- data.frame(
...
nest_survival_reals_BSC_levels <- 
  nest_survival_run_BSC[[36]]$results$real
Groups <- data.frame(
...
nest_survival_reals_FP_habitat <- 
  nest_survival_run_FP[[30]]$results$real
Groups <- data.frame(
...
nest_survival_reals_MP_habitat <- 
  nest_survival_run_MP[[29]]$results$real
Groups <- data.frame(
...
nest_survival_reals_BSC_habitat <- 
  nest_survival_run_BSC[[30]]$results$real
Groups <- data.frame(
...
```{r}
S.dog_m <- nest_survival_run_FP[[24]]
...
S.dog_m <- nest_survival_run_MP[[23]]
# S.dog_m$results$beta
...
```{r}
S.dof_m <- nest_survival_run_FP[[20]]
...
```{r}
S.hum_m <- nest_survival_run_FP[[34]]
...
S.hum_m <- nest_survival_run_MP[[33]]
# S.hum_m$results$beta
...
S.hum_m <- nest_survival_run_BSC[[34]]
# S.hum_m$results$beta
...
## Effect of predatory birds on nest survival (max counted)
```{r}
S.pbd_m <- nest_survival_run_FP[[39]]
...
S.pbd_m <- nest_survival_run_MP[[38]]
# S.pbd_m$results$beta
...
S.pbd_m <- nest_survival_run_BSC[[39]]
# S.pbd_m$results$beta
...
nest_survival_reals_MP_fox_p <- 
  nest_survival_run_MP[[26]]$results$real
Groups <- data.frame(
...
```{r}
S.veh_m <- nest_survival_run_FP[[47]]
...
S.veh_m <- nest_survival_run_MP[[46]]
# S.veh_m$results$beta
...
S.veh_m <- nest_survival_run_BSC[[47]]
# S.veh_m$results$beta
...
nest_survival_reals_FP_season <- 
  nest_survival_run_FP[[42]]$results$real
Groups <- data.frame(
...
nest_survival_reals_BSC_season <- 
  nest_survival_run_BSC[[42]]$results$real
Groups <- data.frame(
